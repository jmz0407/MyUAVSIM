...
class DynamicStdmaEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self._is_valid_assignment(self.current_node, action):
            ...
            logging.info(f"Node {self.current_node} assigned to slot {action}")
            logging.info("current_schedule: ", self.current_schedule)
            # 检查是否完成所有分配
    ...
    def _calculate_reward(self, action):
        ...
        if self.current_requirement and self.current_requirement.routing_path:
            if self.current_node not in self.current_requirement.routing_path:
                return -1  # 非路由节点惩罚
