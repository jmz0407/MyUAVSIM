...
from muti_dynamic_env import MUtiDynamicStdmaEnv
import matplotlib
import logging
...
def train_model(config):
    ...
    env = MUtiDynamicStdmaEnv(num_nodes=start_nodes, max_nodes=max_nodes)
    env = Monitor(env, os.path.join(log_dir, "train"))
    train_env = DummyVecEnv([lambda: env])

    # 创建评估环境
    eval_env_10nodes = MUtiDynamicStdmaEnv(num_nodes=10, max_nodes=max_nodes)
    ...
    eval_env_20nodes = MUtiDynamicStdmaEnv(num_nodes=20, max_nodes=max_nodes)
...
def evaluate_model(model_path, node_sizes=[5, 10, 15, 20, 30], episodes=10):
    ...
    for nodes in node_sizes:
        print(f"\n测试 {nodes} 节点网络...")
        env = MUtiDynamicStdmaEnv(num_nodes=nodes, max_nodes=max_nodes)
        episode_rewards = []
        reuse_ratios = []
        slot_counts = []
