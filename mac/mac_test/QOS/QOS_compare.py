...
for i, traffic in enumerate(traffic_types):
    ...
    ax.set_title(f'{traffic}业务的QoS满足率', fontsize=16)
    ax.set_xlabel('网络规模（节点数）', fontsize=14)
    ax.set_ylabel('QoS满足率 (%)', fontsize=14)
...
# 设置图表属性
ax1.set_title('30节点规模下的性能对比 (GAT-PPO vs TDMA)', fontsize=16)
ax1.set_xticks(index + bar_width / 2)
ax1.set_xticklabels(['GAT-PPO', 'TDMA'])
ax1.set_ylabel('吞吐量和QoS满足率', fontsize=14)
ax1_twin.set_ylabel('延迟 (ms)', fontsize=14)
ax1.set_ylim(0, 100)
...
# 设置图表属性
ax2.set_title('GAT-PPO在不同网络规模下的性能趋势', fontsize=16)
ax2.set_xlabel('网络规模（节点数）', fontsize=14)
ax2.set_ylabel('吞吐量和QoS满足率', fontsize=14)
ax2_twin.set_ylabel('延迟 (ms)', fontsize=14)
