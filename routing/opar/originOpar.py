...
class OriginalOpar:
    """
    原始OPAR协议实现 (基于论文"OPAR: Optimized Predictive and Adaptive Routing for Cooperative UAV Networks")

    属性:
        simulator: 包含所有模拟环境的仿真器对象
        my_drone: 安装该路由协议的无人机
        cost: 成本矩阵，用于记录所有链路的成本
        edge_lifetimes: 链路生命期矩阵，记录所有链路的预测生命期
        best_obj: 所有迭代中的最小目标函数值
        best_path: 对应"best_obj"的最优路由路径
        w1: 目标函数中第一项（路径长度）的权重
        w2: 目标函数中第二项（路径生命期）的权重
        max_comm_range: 对应于信噪比阈值的最大通信范围
    ...
    def __init__(self, simulator, my_drone):
        ...
        self.cost = None  # 成本矩阵
        self.edge_lifetimes = None  # 边的生命期矩阵
        self.best_obj = float('inf')  # 最小目标函数值
        ...
        # 计算最大通信范围
        self.max_comm_range = self.calculate_max_comm_range()

        # 启动等待列表检查进程
        self.simulator.env.process(self.check_waiting_list())

    def calculate_max_comm_range(self):
        """计算最大通信范围"""
        # 这里可以根据您的物理层模型进行具体实现
        # 在原始论文中，这是基于信噪比阈值计算的
        try:
    ...
    def find_optimized_path(self, src_id, dst_id):
        ...
        while path:
            ...
            for i in range(len(path) - 1):
                ...
                # 更新最小生命期
                link_lifetime = self.edge_lifetimes[node1, node2]
                if link_lifetime < minimum_lifetime:
                    minimum_lifetime = link_lifetime

            # 计算目标函数值：w1*路径长度 + w2/最小生命期
            inverse_lifetime = 1.0 / minimum_lifetime if minimum_lifetime > 0 else float('inf')
            obj_value = self.w1 * total_cost + self.w2 * inverse_lifetime
